...
import { ErrorMiddlewareTypeProps, PaginationQuery, TRequest, TResponse } from "../../types";
import { deleteQuickMeal, deleteQuickMealProduct, getQuickMealInfo, getQuickMeals, getUniqueProducts, saveQuickMeal } from "../../network/quick_meals";
...
router.get(ROUTES.QUICK_MEALS.UNIQUE_PRODUCTS, async (req: TRequest<any, PaginationQuery>, res: TResponse<Awaited<ReturnType<typeof getUniqueProducts>>>) => {
    const {uid} = res.locals.user!;
    const {date} = req.query;
    res.json(await getUniqueProducts({date,uid}));
})

router.get(ROUTES.QUICK_MEALS.GET_MEALS, async (req: TRequest<any, PaginationQuery>, res: TResponse<Awaited<ReturnType<typeof getQuickMeals>>>) => {
    const {uid} = res.locals.user!;
    const {date} = req.query;
    res.json(await getQuickMeals({date,uid}));
})

router.get(ROUTES.QUICK_MEALS.GET_MEAL, async (req: TRequest<any, {mid: string}>, res: TResponse<Awaited<ReturnType<typeof getQuickMealInfo>> | ErrorMiddlewareTypeProps>) => {
    const {uid} = res.locals.user!;
    const {mid} = req.query;
...
router.post(ROUTES.QUICK_MEALS.SAVE_MEAL, async (req: TRequest<Omit<QuickMealFull, 'imageUrl'>, any, any>, res: TResponse<{success: boolean}>) => {
    const {uid} = res.locals.user!;
    res.json({success: await saveQuickMeal({uid,meal:req.body})});
})


router.delete(ROUTES.QUICK_MEALS.DELETE_MEAL, async (req: TRequest<{mid: string}>, res: TResponse<{success: boolean}>) => {
    const {uid} = res.locals.user!;
    const {mid} = req.body;
