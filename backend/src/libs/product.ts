...
export const formatProductsSearch = (
  data: any,
): Omit<ProductQueryResponse, 'query'> => {
...
export const getProductDBMeal = async (meal: MealFull): Promise<Meal> => {
  ...
  for (const product of products) {
    ...
    const {_id} = await ProductDBModel.create({
      quantity: product.quantity,
      pid: savedProduct._id,
      addedBy: meal.uid,
    });
...
export const formatProductWithGrams = (
  ...
  return {
    ...
    nutrients: {
      ...
      cholesterol:
        Math.round(savedProduct.nutrients.cholesterol * factor * 100) / 100,
      fiber: Math.round(savedProduct.nutrients.fiber * factor * 100) / 100,
      iron: Math.round(savedProduct.nutrients.iron * factor * 100) / 100,
      potassium:
        Math.round(savedProduct.nutrients.potassium * factor * 100) / 100,
      ...
      carbohydrates:
        Math.round(savedProduct.nutrients.carbohydrates * factor * 100) / 100,
      fat: Math.round(savedProduct.nutrients.fat * factor * 100) / 100,
      saturatedFat:
        Math.round(savedProduct.nutrients.saturatedFat * factor * 100) / 100,
    },
    quantity: quantity,
  };
};
...
export const getNutrientsGrams = (product: Product[]): Nutrients => {
  return {
    fat: Math.round(
      product.reduce((acc, product) => acc + (product?.nutrients?.fat ?? 0), 0),
    ),
    saturatedFat: Math.round(
      product.reduce(
        (acc, product) => acc + (product?.nutrients?.saturatedFat ?? 0),
        0,
      ),
    ),
    protein: Math.round(
      product.reduce(
        (acc, product) => acc + (product?.nutrients?.protein ?? 0),
        0,
      ),
    ),
    carbohydrates: Math.round(
      product.reduce(
        (acc, product) => acc + (product?.nutrients?.carbohydrates ?? 0),
        0,
      ),
    ),
    sugars: Math.round(
      product.reduce(
        (acc, product) => acc + (product?.nutrients?.sugars ?? 0),
        0,
      ),
    ),
    sodium: Math.round(
      product.reduce(
        (acc, product) => acc + (product?.nutrients?.sodium ?? 0),
        0,
      ),
    ),
    fiber: Math.round(
      product.reduce(
        (acc, product) => acc + (product?.nutrients?.fiber ?? 0),
        0,
      ),
    ),
    calcium: Math.round(
      product.reduce(
        (acc, product) => acc + (product?.nutrients?.calcium ?? 0),
        0,
      ),
    ),
    iron: Math.round(
      product.reduce(
        (acc, product) => acc + (product?.nutrients?.iron ?? 0),
        0,
      ),
    ),
    cholesterol: Math.round(
      product.reduce(
        (acc, product) => acc + (product?.nutrients?.cholesterol ?? 0),
        0,
      ),
    ),
    potassium: Math.round(
      product.reduce(
        (acc, product) => acc + (product?.nutrients?.potassium ?? 0),
        0,
      ),
    ),
  };
...
export const getProductsAggregatedCalories = (products: Product[]) => {
  return Math.round(
    products.reduce((acc, product) => acc + (product?.calories ?? 0), 0),
  );
};
...
export const getNutrientsPercentage = (
  product: Product[],
  totalGrams: number,
): Nutrients => {
